...
enum class Result : uint16_t {
  Undefined = 0, // Will be never returned by the reader and can be used the to
                 // indicate preding or empty results
  Success = 1,   // Everything went smooth, the STL file was read or written
                 // without issues
  FileError = 2, // Unable to read or write the specified file path
  MissingDataError =
      3, // STL data ended unexpectely and is incomplete or otherwise broken
  UnexpectedError =
      4,           // Found an unexpected keyword or token in an ASCII STL file
  ParserError = 5, // Unable to parse vertex coordinates or normal vector in an
                   // ASCII STL file
  LineLimitError =
      6, // ASCII line size exceeded internal safety limit of ASCII_LINE_LIMIT
  FacetCountError =
      7, // Binray file exceeds internal safety limit of BINARY_FACET_LIMIT
  EndianError =
      8, // The code currently only supports little endian architectures
...
public:
  ...
  public:
    ...
    // Always called when parsing a binary STL. Before onFacet() is called for
    // the first time
    virtual void onFacetCount(uint32_t triangles) {}

    // May be called when parsing an ASCII STL file with a valid name. Will be
    // always called before onFacet()
    virtual void onName(const std::string &name) {}

    // Return true to force the recalulcation of the normal vector for all
    // facets. By default this is only done for zero normal vectors or normal
    // vectors with an invalid length. This function is only called once before
    // reading the STL data.
    virtual bool forceRecalculateNormals() { return false; }

    // Return true to disable the recalulcation of the normal vector in all
    // cases. By default this is only done for zero normal vectors or normal
    // vectors with an invalid length. This function is only called once before
    // reading the STL data.
    virtual bool disableRecalculateNormals() { return false; }

    // Might be called in ASCII mode when an error is detected to signal the
    // line number of the problem Do not rely on this method to be called when
    // an error occurs, its fully optional!
    virtual void onError(size_t lineNumber) {}

    // Will be called for each triangle (a.k.a facet/face) in the STL file
    virtual void onFacet(const float v1[3], const float v2[3],
                         const float v3[3], const float n[3]) = 0;

    // Can be called for non-zero attribute values of facets in binary STL files
    // after onFacet()
  ...
  static Result readStlFile(const std::filesystem::path &filePath,
                            Handler &handler) {
  ...
  static Result readStlBuffer(const char *buffer, size_t bufferSize,
                              Handler &handler) {
  ...
  static Result readStlStream(std::istream &is, Handler &handler) {
    ...
    Result result = asciiMode ? readAsciiStream(is, handler)
                              : readBinaryStream(is, handler);
    handler.onEnd(result);
    return result;
  }
...
private:
  static bool isAsciiFormat(std::istream &is) {
    // Some CAD applications create binary files that have the string "solid"
    // inside the header. This means we cannot just check the first word, but
    // also need some additional heuristic checks. The checks below are inspired
    // by https://github.com/sreiter/stl_reader/
  ...
  static inline bool stringStartsWith(const std::string &str,
                                      const char *prefix) {
  ...
  static bool stringParseThreeValues(const std::string &str, float &v1,
                                     float &v2, float &v3) {
  ...
  static void calculateNormals(const float v1[3], const float v2[3],
  ...
  static void checkAndFixNormals(const float v1[3], const float v2[3],
                                 const float v3[3], float n[3]) {
  ...
  static Result readAsciiStream(std::istream &is, Handler &handler) {
    ...
    size_t lineNumber = 0, solidCount = 0, facetCount = 0, loopCount = 0,
           vertexCount = 0;
    float n[3] = {
        0,
    };
    ...
    while (true) {
      ...
      if (stringStartsWith(line, "vertex")) {
        ...
        if (!stringParseThreeValues(tmp, v[vertexCount * 3 + 0],
                                    v[vertexCount * 3 + 1],
                                    v[vertexCount * 3 + 2])) {
          handler.onError(lineNumber);
          return Result::ParserError;
  ...
  static Result readBinaryStream(std::istream &is, Handler &handler) {
    ...
    for (size_t t = 0; t < facetCount; t++) {
      ...
      if (buffer[48] != 0 || buffer[49] != 0)
        handler.onFacetAttributes(
            reinterpret_cast<const uint8_t *>(buffer + 48));
    }
  ...
  struct imstream : virtual membuf, std::istream {
    imstream(char const *base, size_t size)
...
class Writer {
public:
  // Provider that must be implemented configure options and provide data when
  // writing STL files
  class Provider {
  public:
    virtual ~Provider() {}
    ...
    virtual void getHeader(uint8_t header[80]) {
      memset(header, 0, 80);
      memcpy(header, libraryName, strlen(libraryName));
    }

    // Return true to write nulled out normals, return false write existing
    // normal data
    virtual bool nullifyNormals() { return false; }

    // Return true if you want to write custom attribute values in binary STL
    // files using getFacetAttributes()
    ...
    // Will be called once for each facet/triangle with its corresponding zero
    // based index The arrays v1, v2, v3 and n are output parameters
    virtual void getFacet(size_t index, float v1[3], float v2[3], float v3[3],
                          float n[3]) = 0;

    /// Will be called once for each facet/triangle after getFacet() if
    /// writeAttributes() is true
    // The array attributes is an output parameter
    virtual void getFacetAttributes(size_t index, uint8_t attributes[2]) {
      memset(attributes, 0, 2);
    }
  };
  ...
  static Result writeStlFile(const std::filesystem::path &filePath,
                             Provider &provider) {
  ...
  static Result writeStlStream(std::ostream &os, Provider &provider) {
    ...
    Result result = asciiMode ? writeAsciiStream(os, provider)
                              : writeBinaryStream(os, provider);
    return result;
  }
...
inline std::string getResultString(Result result) {
  ...
  static_assert(sizeof(Result) == sizeof(uint16_t),
                "Please adjust the code below with new type!");
  const uint16_t knowLastValue = 9u;
  const uint16_t currentLastValue =
      static_cast<uint16_t>(Result::__LAST__RESULT__VALUE);
  static_assert(knowLastValue == currentLastValue,
                "Please extend the switch cases!");
  switch (result) {
...
struct MeshReaderHandler : Reader::Handler {
  ...
  void onBegin(bool m) override {
    clear();
    ascii = m;
  }
  void onBinaryHeader(const uint8_t buffer[80]) override {
    header.resize(80);
    memcpy(header.data(), buffer, 80);
  }
  ...
  void onFacet(const float v1[3], const float v2[3], const float v3[3],
               const float n[3]) override {
    Facet facet;
...
struct MeshProvider : microstl::Writer::Provider {
  ...
  void getFacet(size_t index, float v1[3], float v2[3], float v3[3],
                float n[3]) override {
...
struct FVMeshProvider : microstl::Writer::Provider {
  ...
  void getFacet(size_t index, float v1[3], float v2[3], float v3[3],
                float n[3]) override {
    const auto &facet = mesh.facets[index];
    v1[0] = mesh.vertices[facet.v1].x;
    v1[1] = mesh.vertices[facet.v1].y;
    v1[2] = mesh.vertices[facet.v1].z;
    v2[0] = mesh.vertices[facet.v2].x;
    v2[1] = mesh.vertices[facet.v2].y;
    v2[2] = mesh.vertices[facet.v2].z;
    v3[0] = mesh.vertices[facet.v3].x;
    v3[1] = mesh.vertices[facet.v3].y;
    v3[2] = mesh.vertices[facet.v3].z;
    n[0] = facet.n.x;
    n[1] = facet.n.y;
    n[2] = facet.n.z;
  }
...
inline FVMesh deduplicateVertices(const Mesh &inputMesh) {
  FVMesh outputMesh;
  auto addVertex = [&outputMesh](const Vertex &v) {
    for (size_t i = 0; i < outputMesh.vertices.size(); i++) {
      if (v.x == outputMesh.vertices[i].x && v.y == outputMesh.vertices[i].y &&
